...
public class ObjectPooling<T>
{
    ...
    public ObjectPooling(Func<T> factoryMethod, Action<T> turnOnCallback, Action<T> turnOffCallback, int initialStock = 0, bool isDynamic = true)
    {
    ...
    {
        if (_activeStock.Count == 0)
        {
            return default(T);
        }
        
        int index = UnityEngine.Random.Range(0, _activeStock.Count);
        T objToReturn = _activeStock[index];
        _turnOffCallback(objToReturn);
        _inactiveStock.Add(objToReturn);
        _activeStock.RemoveAt(index);
